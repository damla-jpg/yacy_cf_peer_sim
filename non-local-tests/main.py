...
# Port configurations
BASE_YACY_PORT = 8090  # Port for the server (starting from 8092)
BASE_YACY_SSL_PORT = 8443  # Port for the SSL server (starting from 8444)
BASE_SOCKET_PORT = 8190  # Port for the socket server (starting from 8192)
BASE_BACKEND_PORT = 3000  # Port for the backend (starting from 3002)
BASE_FRONTEND_PORT = 4000  # Port for the frontend (starting from 4002)
...
CLOSING_TIME = 300  # Duration (in seconds) to run the simulation
...
class PeerSimulator:
    ...
    def check_if_non_local_senior_peer(self, index):
        """
        Check if non-local is a senior peer
        """

        # Setup the URL and XPath
        url = f"http://80.112.187.103:{BASE_YACY_PORT + index}/Status.html"
    ...
    def create_shells(self, index):
        ...
        with open("start_peer.sh", "r", encoding="UTF-8") as file:
            data = file.read()
    ...
    def wait_for_senior_peer(self, index):
        ...
        with tqdm(
            ...
            for i in range(SENIOR_CHECK_TIMEOUT):
                ...
                result_non_local = self.check_if_non_local_senior_peer(index)
                if "senior" in result.lower() and "senior" in result_non_local.lower():
