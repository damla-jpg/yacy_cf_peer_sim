...
if [ ! -z "$2" ] && [ "$2" == "0" ]; then
    ...
    docker exec yacy-server-0 sh -c "cd /opt/yacy_search_server/DATA/SETTINGS && sed -i 's/port=8090/port=8090/' yacy.conf && sed -i 's/port.ssl=8443/port.ssl=8443/' yacy.conf"
